...
type SemanticButtonProperties = BaseButtonProperties & Omit<React.ComponentPropsWithoutRef<'button'>, 'type'> & {
	type?: 'button' | 'submit';
};
...
const Semantic = ({
	label, variant = 'primary', size = 'regular', className, icon, type = 'button', ...intrinsicButtonProperties
	...
	return (
		<button className={classes.root} type={type === 'button' ? 'button' : 'submit'} {...intrinsicButtonProperties}>
			{icons.left}
			<span>{label}</span>
			{icons.right}
		</button>
